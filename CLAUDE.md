...
1. **Injected doc blocks** — `strip_kapsis_injections()` (`scripts/post-container-git.sh`) removes the gist-instructions block that `inject_gist_instructions()` appended to `CLAUDE.md`/`AGENTS.md`. Blocks are bracketed by HTML-comment sentinels defined in `scripts/lib/constants.sh` (`KAPSIS_GIST_MARKER_BEGIN`/`_END`). Matching tolerates CRLF; unbalanced markers leave the file unchanged with a warning. A legacy fallback strips pre-sentinel blocks (`---` separator + `# Kapsis Activity Gist` heading at EOF). The stripped file count is reported in `status.json` as `stripped_injections`.
