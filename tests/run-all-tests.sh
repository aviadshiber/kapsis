...
get_tests_for_category() {
    ...
    case "$category" in
        agent)
            ...
            ;;
        validation)
            echo "test-input-validation.sh test-path-spaces.sh test-dry-run-completeness.sh test-preflight-check.sh test-version-fetch.sh test-version-management.sh test-post-container-exit-code.sh test-disk-space-guard.sh test-workspace-mount-validation.sh"
            ;;
        status)
