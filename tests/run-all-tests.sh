...
get_tests_for_category() {
    ...
    case "$category" in
        ...
        validation)
            ...
            ;;
        status)
            echo "test-status-reporting.sh test-status-hooks.sh test-e2e-gist-hook.sh test-e2e-all-hooks.sh test-e2e-claude-hooks-real.sh"
            ;;
        filesystem)
