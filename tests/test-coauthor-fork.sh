...
test_config_attribution_parsing() {
    ...
    if ! command -v yq &>/dev/null; then
        log_info "yq not installed, skipping attribution parsing test"
        return 0
    fi
...
Branch: {branch}"
    ...
    if [[ "$result" == *"{version}"* || "$result" == *"{agent_id}"* || "$result" == *"{branch}"* ]]; then
        log_error "Unsubstituted placeholders remain in: $result"
        return 1
    fi
    log_info "All placeholders substituted correctly"
}
...
[Generated by Kapsis](https://github.com/aviadshiber/kapsis) v1.0"
    ...
    if [[ "$count" -ne 1 ]]; then
        log_error "Expected exactly 1 'Generated by Kapsis' occurrence, got: $count"
        log_error "Commit message: $last_msg"
        return 1
    fi
    log_info "Claude attribution correctly deduplicated"
}
...
test_cli_co_author_format_validation() {
    ...
    # The same regex used in launch-agent.sh parse_args
    local valid_inputs=(
