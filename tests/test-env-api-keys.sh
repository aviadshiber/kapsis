...
#===============================================================================
# SECRET STORE INJECTION TESTS (Issue #162)
#===============================================================================

test_secret_store_entries_metadata() {
    log_test "Testing KAPSIS_SECRET_STORE_ENTRIES metadata format"

    # Simulate the format generated by launch-agent.sh for secret store entries
    # shellcheck disable=SC2178,SC2128
    local entries=""
    local items=(
        "BITBUCKET_TOKEN|bitbucket-service|aviad.s"
        "GIT_TOKEN|git-service|kapsis"
    )
    ...
    # Verify format: VAR_NAME|service|account (comma-separated)
    assert_contains "$entries" "BITBUCKET_TOKEN|bitbucket-service|aviad.s" "Should contain BITBUCKET_TOKEN entry"
    assert_contains "$entries" "GIT_TOKEN|git-service|kapsis" "Should contain GIT_TOKEN entry"

    # Verify comma separation
    IFS=',' read -ra parsed <<< "$entries"
    assert_equals "2" "${#parsed[@]}" "Should have 2 entries"

    # Verify pipe-separated fields
    IFS='|' read -r var_name service account <<< "${parsed[0]}"
    assert_equals "BITBUCKET_TOKEN" "$var_name" "First field should be var name"
    assert_equals "bitbucket-service" "$service" "Second field should be service"
    assert_equals "aviad.s" "$account" "Third field should be account"
}

test_inject_secret_store_fallback() {
    log_test "Testing inject_secret_store graceful fallback when secret-tool unavailable"

    # Source the entrypoint's inject_secret_store function
    # Since secret-tool is not available on macOS host, it should log a warning and return 0
    local entrypoint_script="$KAPSIS_ROOT/scripts/entrypoint.sh"

    # Verify the function exists in the entrypoint
    assert_contains "$(cat "$entrypoint_script")" "inject_secret_store" "entrypoint should contain inject_secret_store function"

    # Verify it checks for secret-tool availability
    assert_contains "$(cat "$entrypoint_script")" "command -v secret-tool" "Should check for secret-tool"

    # Verify it handles missing gnome-keyring-daemon
    assert_contains "$(cat "$entrypoint_script")" "gnome-keyring-daemon" "Should reference gnome-keyring-daemon"

    # Verify it unsets KAPSIS_SECRET_STORE_ENTRIES after processing
    assert_contains "$(cat "$entrypoint_script")" "unset KAPSIS_SECRET_STORE_ENTRIES" "Should clean up metadata env var"
}

test_inject_secret_store_function_exists() {
    log_test "Testing inject_secret_store function is called in entrypoint"

    local entrypoint_script="$KAPSIS_ROOT/scripts/entrypoint.sh"

    # Verify the function is called (not just defined) after inject_credential_files
    local call_line
    call_line=$(grep -n "inject_secret_store$" "$entrypoint_script" | grep -v "^.*inject_secret_store()" | head -1) || true

    assert_not_equals "" "$call_line" "inject_secret_store should be called in entrypoint"

    # Verify inject_credential_files is called before inject_secret_store
    local cred_line
    cred_line=$(grep -n "inject_credential_files$" "$entrypoint_script" | head -1 | cut -d: -f1)
    local ss_line
    ss_line=$(grep -n "inject_secret_store$" "$entrypoint_script" | grep -v "()" | head -1 | cut -d: -f1)

    if [[ -n "$cred_line" && -n "$ss_line" ]]; then
        assert_true "[[ $cred_line -lt $ss_line ]]" "inject_credential_files should be called before inject_secret_store"
    fi
}
...
main() {
    ...
    # Secret store injection tests (Issue #162)
    run_test test_secret_store_entries_metadata
    run_test test_inject_secret_store_fallback
    run_test test_inject_secret_store_function_exists
