...
generate_process_isolation_args() {
    ...
    # User namespace (always enabled). Resolution order:
    #   1. KAPSIS_USERNS env var  (debug override; highest precedence)
    #   2. SECURITY_USERNS         (parsed from agent YAML's security.userns)
    #   3. detect_userns_default   (autodetect from host UID — see #361)
    #
    # Plain --userns=keep-id breaks on hosts where `id -u` is far outside the
    # podman VM's /etc/subuid range (e.g. macOS AD/LDAP domain accounts whose
    # UIDs are typically > 1 billion). Podman's keep-id resolver intermittently
    # emits a degenerate single-ID mapping that excludes UID 1000, causing
    # container attach to fail with exit 126:
    #   "container uses ID mappings (...), but doesn't map UID 1000"
    # detect_userns_default() handles this by switching to the explicit
    # `keep-id:uid=1000,gid=1000` form for high-UID hosts. File ownership on
    # bind-mounted dirs is unchanged (verified — host UID is preserved on
...
# Exposed as a helper so it can be stubbed in bats tests.
_resolve_userns() {
...
#
# Threshold = 60000 (POSIX UID_MAX convention). Standard Unix accounts (root,
# system daemons, local users) sit below this; AD/LDAP/Kerberos domain accounts
# generated by macOS DirectoryService routinely exceed it (verified: a Taboola
# corporate Mac account had UID 1882662165 = 0x70372515).
#
# - host UID ≤ 60000 → plain `keep-id` (works because the host UID fits
#                       cleanly inside the podman VM's subuid range).
# - host UID > 60000 → `keep-id:uid=1000,gid=1000` (explicit form tells podman
#                       to map the container's developer user regardless of
#                       what the host UID is). Without the explicit form,
#                       podman's resolver intermittently emits a degenerate
#                       single-ID mapping → exit 126 at container attach.
#
# Override-style: callers can pin `security.userns:` in YAML to skip detection,
# or export KAPSIS_USERNS for a session-local override.
_detect_userns_default() {
    local host_uid
    # `id -u` is overridable in bats tests by defining a shell function `id`
    # in the test setup — see tests/unit/test-userns-resolution.sh.
