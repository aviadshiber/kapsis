...
if [[ -n "$CONFIG_FILE" ]] && [[ -f "$SCRIPT_DIR/lib/build-config.sh" ]]; then
    ...
    if parse_build_config "$CONFIG_FILE"; then
        ...
        BUILD_ARGS+=("--build-arg" "ENABLE_MAVEN_EXTENSIONS=$ENABLE_MAVEN_EXTENSIONS")
        ...
        [[ -n "${MAVEN_EXTENSIONS:-}" ]] && BUILD_ARGS+=("--build-arg" "MAVEN_EXTENSIONS=$MAVEN_EXTENSIONS")
        [[ -n "${MAVEN_EXTENSIONS_VULNERABLE_PATHS:-}" ]] && BUILD_ARGS+=("--build-arg" "MAVEN_EXTENSIONS_VULNERABLE_PATHS=$MAVEN_EXTENSIONS_VULNERABLE_PATHS")
