...
if [[ -f "$SDKMAN_DIR/bin/sdkman-init.sh" ]]; then
    # Disable strict mode for ALL SDKMAN operations - SDKMAN references
    # various unset variables (ZSH_VERSION, SDKMAN_OFFLINE_MODE, etc.)
...
# Note: set +u remains active for sdk calls below (SDKMAN uses unset variables)
case "$VERSION" in
    8|1.8)
