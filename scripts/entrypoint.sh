...
# git_push_refspec, verify_git_push — eliminates duplication with post-container-git.sh)
if [[ -f "$KAPSIS_HOME/lib/git-operations.sh" ]]; then
    source "$KAPSIS_HOME/lib/git-operations.sh"
elif [[ -f "$(dirname "${BASH_SOURCE[0]}")/lib/git-operations.sh" ]]; then
    source "$(dirname "${BASH_SOURCE[0]}")/lib/git-operations.sh"
fi
...
#
# Split into sub-functions for single-responsibility compliance.
# Delegates change detection and push verification to git-operations.sh.
# Reads: KAPSIS_BRANCH, KAPSIS_GIT_REMOTE, KAPSIS_REMOTE_BRANCH,
#   KAPSIS_DO_PUSH, KAPSIS_TASK, KAPSIS_AGENT_ID
#===============================================================================
post_exit_git() {
    if [[ -z "${KAPSIS_BRANCH:-}" ]]; then
        return
    ...
    # Delegate to git-operations.sh for change detection
    if has_git_changes; then
        has_uncommitted=true
    fi
    if has_unpushed_commits "$remote" "$remote_branch"; then
        has_unpushed=true
    fi
    ...
    # Push if enabled, otherwise show manual push instructions
    if [[ "${KAPSIS_DO_PUSH:-false}" == "true" ]]; then
        _post_exit_push "$remote" "$remote_branch"
    else
        _post_exit_no_push "$remote" "$remote_branch"
    fi
}

# Stages and commits all uncommitted changes
# Reads: KAPSIS_TASK, KAPSIS_AGENT_ID, KAPSIS_BRANCH
_post_exit_commit() {
    echo ""
    echo "┌────────────────────────────────────────────────────────────────┐"
    ...
    task_summary="${task_summary:0:72}"
...
# Pushes to remote and verifies. Delegates to git-operations.sh.
# Arguments: $1 - remote, $2 - remote_branch
# Reads: KAPSIS_BRANCH
_post_exit_push() {
    local remote="$1"
    ...
    git_push_refspec "$remote" "${KAPSIS_BRANCH}" "$remote_branch" || {
        log_warn "Push failed. Changes are committed locally."
        _git_ops_set_push_info "failed" "$local_commit" ""
        return
    }

    # Delegate verification to git-operations.sh
    echo ""
    local verify_result=0
    verify_git_push "$remote" "$remote_branch" || verify_result=$?

    if [[ $verify_result -le 2 ]]; then
        # 0=verified, 2=unverifiable (network issue) — both acceptable
        _post_exit_show_pr "$remote" "$remote_branch"
        echo ""
...
# Shows PR URL after successful push
# Arguments: $1 - remote, $2 - remote_branch
_post_exit_show_pr() {
    local remote="$1"
    local remote_branch="$2"
...
# Shows manual push instructions when --push is not enabled
# Arguments: $1 - remote, $2 - remote_branch
# Reads: KAPSIS_BRANCH
_post_exit_no_push() {
    local remote="$1"
    ...
    echo "To push later: git push ${remote} ${KAPSIS_BRANCH}:${remote_branch}"
