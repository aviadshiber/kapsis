...
# Example: GIT_TOKEN|internal-git-token|developer,API_TOKEN|api-service|kapsis
#===============================================================================
inject_secret_store() {
...
#===============================================================================
write_git_credential_map() {
    if [[ -z "${KAPSIS_GIT_CREDENTIAL_MAP_DATA:-}" ]]; then
        return 0
    fi
...
find_git_credential_helper() {
    if [[ -x "/opt/kapsis/git-credential-keyring" ]]; then
        echo "/opt/kapsis/git-credential-keyring"
    elif [[ -x "/usr/local/bin/git-credential-keyring" ]]; then
        echo "/usr/local/bin/git-credential-keyring"
    fi
}

git_config_safe_value() {
    local value="$1"
...
append_safe_git_config_value() {
    ...
    value=$(git config --file "$source_config" --get "$config_key" 2>/dev/null || true)
    git_config_safe_value "$value" || return 0

    local email_pattern='^[^[:space:]<>]+@[^[:space:]<>]+$'
    if [[ "$config_key" == "user.email" && ! "$value" =~ $email_pattern ]]; then
        return 0
    fi

    git config --file "$target_config" "$config_key" "$value"
}

setup_managed_git_config() {
    local policy="${KAPSIS_GIT_TRANSPORT_POLICY:-inherit}"
    [[ "$policy" == "managed_https" ]] || return 0

    write_git_credential_map

    local helper_path
    helper_path=$(find_git_credential_helper)
    local map_file="${KAPSIS_GIT_CREDENTIAL_MAP:-}"
    local managed_dir="$HOME/.kapsis"
    local managed_config="$managed_dir/gitconfig-managed"
    local tmp_file

    mkdir -p "$managed_dir"
    tmp_file=$(mktemp "${managed_config}.XXXXXX") || {
        log_warn "Could not create managed gitconfig"
        return 0
    }

    {
        echo "# Generated by Kapsis. Contains no secrets."
        echo "[credential]"
        echo "    helper ="
    ...
    if [[ -f "$HOME/.gitconfig" ]]; then
        append_safe_git_config_value "$HOME/.gitconfig" "$tmp_file" "user" "name"
        append_safe_git_config_value "$HOME/.gitconfig" "$tmp_file" "user" "email"
        append_safe_git_config_value "$HOME/.gitconfig" "$tmp_file" "init" "defaultBranch"
        # FIX-2: preserve KAPSIS_DISABLE_HOOKS (core.hooksPath) into the managed config —
        # configure_git_hooks() writes it to $HOME/.gitconfig BEFORE this function
        # redirects GIT_CONFIG_GLOBAL, so without this copy it would be silently lost.
        append_safe_git_config_value "$HOME/.gitconfig" "$tmp_file" "core" "hooksPath"
    fi

    if [[ -n "$helper_path" && -n "$map_file" && -f "$map_file" ]]; then
        while IFS='|' read -r map_host _map_service _map_account _map_collection _map_profile; do
            [[ "${map_host:-}" == "#"* ]] && continue
            [[ -z "${map_host:-}" ]] && continue
            if [[ "$map_host" =~ ^[A-Za-z0-9._-]+$ ]]; then
                {
                ...
                log_warn "Ignoring invalid git credential map host: $map_host"
            fi
        done < "$map_file"
    elif [[ -n "$map_file" && -f "$map_file" ]]; then
        log_warn "git-credential-keyring not found; managed HTTPS git credentials will not be available"
    fi

    chmod 600 "$tmp_file"
    mv "$tmp_file" "$managed_config"
    chmod 600 "$managed_config"

    export GIT_CONFIG_GLOBAL="$managed_config"
    export GIT_CONFIG_NOSYSTEM=1
    export GIT_TERMINAL_PROMPT=0

    log_info "Git transport policy: managed_https"
    log_debug "Using managed gitconfig: $managed_config"
}
...
post_exit_git() {
    ...
    if [[ "$has_uncommitted" == "true" ]]; then
        ...
        # Resolve {worktree} if still present (fallback path).
        ...
        attribution_block="${attribution_block//\{worktree\}/workspace}"
...
validate_workspace_mount() {
    ...
    if [[ "$sandbox_mode" == "worktree" ]]; then
        ...
        if [[ ! -e "$workspace/.git" ]] && [[ ! -d "$git_safe" ]]; then
            log_warn "WORKSPACE MOUNT DEGRADED: No .git file and no sanitized git at $git_safe"
        fi
    fi

    log_debug "Workspace mount validation passed: $workspace"
    return 0
}
...
main() {
    ...
    # Configure git credentials for macOS-to-Linux bridging (Issue #188).
    # Must run AFTER inject_secret_store (secrets in keyring) and
    # AFTER setup_staged_config_overlays (gitconfig is writable copy).
    # FIX-7: this dispatch stays a literal case (bash can't loop over the
    # KAPSIS_GIT_TRANSPORT_POLICIES constant here without extra plumbing),
    # but its branches (managed_https/ssh/inherit) must stay in sync with
    # KAPSIS_GIT_TRANSPORT_POLICIES in scripts/lib/constants.sh — that
    # constant is the validated source of truth used by launch-agent.sh and
    # config-verifier.sh.
    case "${KAPSIS_GIT_TRANSPORT_POLICY:-inherit}" in
        managed_https)
            setup_managed_git_config
            ;;
        ssh)
            log_info "Git transport policy: ssh"
            # FIX-5: neither managed_https nor inherit's path applies here, so
            # KAPSIS_GIT_CREDENTIAL_MAP_DATA would otherwise linger unconsumed
            # in the agent environment. It's metadata only (no secrets), but
            # unset it for hygiene/consistency with the other branches.
            unset KAPSIS_GIT_CREDENTIAL_MAP_DATA
            ;;
        inherit|"")
            patch_git_credential_helpers
            ;;
        *)
            log_warn "Unknown KAPSIS_GIT_TRANSPORT_POLICY '${KAPSIS_GIT_TRANSPORT_POLICY:-}'; using inherit"
            patch_git_credential_helpers
            ;;
    esac

    # Materialize declarative SSH identities (deploy keys), if configured.
    # Independent of git.transport_policy (applies always, like ssh.verify_hosts).
    # Must run after the transport_policy case block (above) so ~/.ssh staging
    # from setup_staged_config_overlays is already in place and the container
    # keyring is up, matching where inject_secret_store() has already run.
    local ssh_identities_lib="${KAPSIS_HOME:-/opt/kapsis}/lib/ssh-identities.sh"
    if [[ -f "$ssh_identities_lib" ]]; then
